...
export default function CourseChapter() {
    ...
    const onClickNext=(index)=>{
        ...
        {   
            let coursePro;
            // Services.getCourseProgress(param.courseId.toString()).then(resp=>{
            //     console.log(resp)
            //     let coursePro=resp;
            //     if(coursePro==null)
            //     {
            //         coursePro=[];
            //     } 
            //     Services.setCourseProgress(param.courseId.toString(),
            //     coursePro.push(""+param.courseContent.id.toString()))
            // })
            const data={
                "data":{
                    "uid":userData.id,
      ...
      <TouchableOpacity onPress={()=>navigation.goBack()}>
        ...
        renderItem={({item,index})=>(
            ...
            marginRight:15,padding:10}}>
                ...
                <Text style={{lineHeight:22}}>{item.description}</Text>
               {item.input!=''?
               <View>
             ...
             color={Colors.white} />
               ...
               {index+1!=chapter.length? <TouchableOpacity 
                ...
                padding:10,borderRadius:7,position:'absolute',bottom:10,
                width:'110%'}}>
